[TOC]
# UnityFrameTools
总结遇到的Unity框架以及其中一些非常好用的组件。根据我的目前情况进行了一定的取舍，把其中模块化的功能进行提取。提取的模块一般只包含最基础的功能，扩展功能需依据具体项目具体设计。
## 一、关键模块
### 1、单例系统
普通单例 以及Mono单例。
直接将Base文件夹复制到新项目，使用时只需要继承泛型单例即可。大多数模块都需要单例模式，
### 2、资源管理工具
...
PS.下面的模块默认都已经导入单例模块、资源管理器和Mono管理器。
### 4、事件系统
一个分发消息、响应消息的系统。
#### 消息系统一【简易版】
简易版更轻量级
